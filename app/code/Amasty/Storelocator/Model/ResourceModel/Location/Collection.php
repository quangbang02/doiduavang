...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    public function prepareRequestParams($attributesData)
    {
        ...
        foreach ($attributesData as $value) {
            if (!empty($value['value']) || $value['value'] != '') {
                $params[$value['name']][] = (int)$value['value'];
            }
        }
    ...
    public function applyAttributeFilters($params)
    {
        foreach ($params as $attributeId => $value) {
            ...
            if(is_int($attributeId) && $attributeId >0){
              ...
              if($attributeId == 'region_id'){
                $this->addRegionToSelect($attributeId, $value);
              }
            }
        }
    ...
    public function addRegionToSelect($attributeId, $value)
    {
        if (is_array($value)) {
            $orWhere = [];
            foreach ($value as $optionId) {
                if (!empty($optionId) || $optionId != '0') {
                    $orWhere[] = "state = ".$optionId;
                }
            }
            if ($orWhere) {
